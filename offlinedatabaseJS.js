...
function createTables() {
    var query = 'CREATE TABLE IF NOT EXISTS devsdofuturo(id INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT, nome VARCHAR NOT NULL, idade VARCHAR NOT NULL);';
    try {
        localDB.transaction(function (transaction) {
            transaction.executeSql(query, [], nullDataHandler, errorHandler);
            updateStatus("Tabela 'devsdofuturo' status: OK.");
        });
    }
    catch (e) {
        updateStatus("Erro: Data base 'devsdofuturo' não criada " + e + ".");
        return;
    }
}
...
function onUpdate() {
    ...
    if (nome == "" || idade == "") {
        ...
        var query = "update devsdofuturo set nome=?, idade=? where id=?;";
        try {
            localDB.transaction(function (transaction) {
...
function onDelete() {
    var id = document.itemForm.id.value;
    var query = "delete from devsdofuturo where id=?;";
    try {
        localDB.transaction(function (transaction) {
...
function onCreate() {
    ...
    if (nome == "" || idade == "") {
        ...
        var query = "insert into devsdofuturo (nome, idade) VALUES (?, ?);";
        try {
            localDB.transaction(function (transaction) {
...
function onSelect(htmlLIElement) {
    ...
    query = "SELECT * FROM devsdofuturo where id=?;";
    try {
        localDB.transaction(function (transaction) {
            transaction.executeSql(query, [id], function (transaction, results) {
...
function queryAndUpdateOverview() {
    ...
    var query = "SELECT * FROM devsdofuturo;";
    try {
        localDB.transaction(function (transaction) {
